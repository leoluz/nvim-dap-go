...
local function get_testfunc_name(str)
    local pattern = "%s(Test%w+)%s*%("
    local substring = string.match(str, pattern)

    return substring
end

local function get_testsuite_name(str)
    local pattern = "%(%w+%s*%*?(%w+)%).-"
    local prefix = "Test"
    local substring = string.match(str, pattern)
...
local function get_testcase()
    local cursor_line = vim.api.nvim_get_current_line()

    local case_name = get_testfunc_name(cursor_line)
    local case_suite = get_testsuite_name(cursor_line)
...
local function get_params(testname)
    -- Running tool:
    -- /usr/bin/go test -timeout 30s -run ^TestSuiteName$
    -- -testify.m ^(TestFuncName)$ ./path/to/package.go
    local name, test_args

    if testname then
        name = testname
        test_args = { "-test.run", "^" .. name .. "$" }
    else
...
M.closest_test = function()
    ...
    local test_case = get_params(get_closest_test())

    if test_case.name then
        test_scope = "testcase"
    else
        test_scope = "package"
    end
